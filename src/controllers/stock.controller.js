...
async function obtenerAlertasVencimiento(req, res) {
    try {
        const alertas =
            await stockService.obtenerAlertasVencimiento();

        res.json(alertas);

    } catch (error) {
        console.error(
            'Error al obtener alertas de vencimiento:',
            error.message
        );

        res.status(500).json({
            error: 'Error al obtener las alertas de vencimiento'
        });
    }
}
